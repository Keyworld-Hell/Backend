...
@Service
public class SwitchLockEngService {

    @Autowired
    private SwitchLockEngRepository repository;

    public List<SwitchLockEng> getAll() {
        return repository.findAll();
    }

    public List<SwitchLockEng> findSwitchLockEngsByTitle(String title) {
        return repository.findByTitleContainingIgnoreCase(title);
    }
    public byte[] getByIdAndFileIndex(Long id, int fileIndex) {
        SwitchLockEng entity = getById(id);
        switch (fileIndex) {
            case 1:
    ...
    public SwitchLockEng getById(Long id) {
        return repository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException());
    }


    public SwitchLockEng save(SwitchLockEngDTO dto) throws IOException {
        SwitchLockEng entity = new SwitchLockEng();
    ...
    public SwitchLockEng update(Long id, SwitchLockEngDTO dto) throws IOException {
        SwitchLockEng entity = getById(id);
        entity.setTitle(dto.getTitle());
        entity.setMaterial(dto.getMaterial());
        entity.setSurface(dto.getSurface());
