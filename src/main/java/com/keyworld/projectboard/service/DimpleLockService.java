...
import com.keyworld.projectboard.dto.DimpleLockDTO;
import com.keyworld.projectboard.repository.DimpleLockRepository;
...
@Service
public class DimpleLockService {

    @Autowired
    private DimpleLockRepository repository;

    public List<DimpleLock> getAll() {
        return repository.findAll();
    }

    public List<DimpleLock> findDimpleLocksByTitle(String title) {
        return repository.findByTitleContainingIgnoreCase(title);
    }
    public byte[] getByIdAndFileIndex(Long id, int fileIndex) {
        DimpleLock entity = getById(id);
        switch (fileIndex) {
            case 1:
    ...
    public DimpleLock getById(Long id) {
        return repository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException());
    }


    public DimpleLock save(DimpleLockDTO dto) throws IOException {
        DimpleLock entity = new DimpleLock();
    ...
    public DimpleLock update(Long id, DimpleLockDTO dto) throws IOException {
        DimpleLock entity = getById(id);
        entity.setTitle(dto.getTitle());
        entity.setMaterial(dto.getMaterial());
        entity.setSurface(dto.getSurface());
