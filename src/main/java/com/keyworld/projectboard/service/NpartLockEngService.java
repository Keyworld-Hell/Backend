...
@Service
public class NpartLockEngService {

    @Autowired
    private NpartLockEngRepository repository;

    public List<NpartLockEng> getAll() {
        return repository.findAll();
    }

    public List<NpartLockEng> findNpartLockEngsByTitle(String title) {
        return repository.findByTitleContainingIgnoreCase(title);
    }
    public byte[] getByIdAndFileIndex(Long id, int fileIndex) {
        NpartLockEng entity = getById(id);
        switch (fileIndex) {
            case 1:
    ...
    public NpartLockEng getById(Long id) {
        return repository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException());
    }


    public NpartLockEng save(NpartLockEngDTO dto) throws IOException {
        NpartLockEng entity = new NpartLockEng();
    ...
    public NpartLockEng update(Long id, NpartLockEngDTO dto) throws IOException {
        NpartLockEng entity = getById(id);
        entity.setTitle(dto.getTitle());
        entity.setMaterial(dto.getMaterial());
        entity.setSurface(dto.getSurface());
