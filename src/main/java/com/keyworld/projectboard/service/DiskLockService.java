...
public class DiskLockService {
    ...
    @Autowired
    private DiskLockRepository repository;

    public List<DiskLock> getAll() {
        return repository.findAll();
    }

    public List<DiskLock> findDiskLocksByTitle(String title) {
        return repository.findByTitleContainingIgnoreCase(title);
    }
    public byte[] getByIdAndFileIndex(Long id, int fileIndex) {
        DiskLock entity = getById(id);
        switch (fileIndex) {
            case 1:
    ...
    public DiskLock getById(Long id) {
        return repository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException());
    }


    public DiskLock save(DiskLockDTO dto) throws IOException {
        DiskLock entity = new DiskLock();
    ...
    public DiskLock update(Long id, DiskLockDTO dto) throws IOException {
        DiskLock entity = getById(id);
        entity.setTitle(dto.getTitle());
        entity.setMaterial(dto.getMaterial());
        entity.setSurface(dto.getSurface());
