...
@Service
public class HingeEngService {

    @Autowired
    private HingeEngRepository repository;

    public List<HingeEng> getAll() {
        return repository.findAll();
    }

    public List<HingeEng> findHingeEngsByTitle(String title) {
        return repository.findByTitleContainingIgnoreCase(title);
    }
    public byte[] getByIdAndFileIndex(Long id, int fileIndex) {
        HingeEng entity = getById(id);
        switch (fileIndex) {
            case 1:
    ...
    public HingeEng getById(Long id) {
        return repository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException());
    }


    public HingeEng save(HingeEngDTO dto) throws IOException {
        HingeEng entity = new HingeEng();
    ...
    public HingeEng update(Long id, HingeEngDTO dto) throws IOException {
        HingeEng entity = getById(id);
        entity.setTitle(dto.getTitle());
        entity.setMaterial(dto.getMaterial());
        entity.setSurface(dto.getSurface());
