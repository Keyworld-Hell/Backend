...
@Service
public class SpecialEngService {

    @Autowired
    private SpecialEngRepository repository;

    public List<SpecialEng> getAll() {
        return repository.findAll();
    }

    public List<SpecialEng> findSpecialEngsByTitle(String title) {
        return repository.findByTitleContainingIgnoreCase(title);
    }
    public byte[] getByIdAndFileIndex(Long id, int fileIndex) {
        SpecialEng entity = getById(id);
        switch (fileIndex) {
            case 1:
    ...
    public SpecialEng getById(Long id) {
        return repository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException());
    }


    public SpecialEng save(SpecialEngDTO dto) throws IOException {
        SpecialEng entity = new SpecialEng();
    ...
    public SpecialEng update(Long id, SpecialEngDTO dto) throws IOException {
        SpecialEng entity = getById(id);
        entity.setTitle(dto.getTitle());
        entity.setMaterial(dto.getMaterial());
        entity.setSurface(dto.getSurface());
