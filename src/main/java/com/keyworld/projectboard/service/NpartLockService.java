...
@Service
public class NpartLockService {

    @Autowired
    private NpartLockRepository repository;

    public List<NpartLock> getAll() {
        return repository.findAll();
    }

    public List<NpartLock> findNpartLocksByTitle(String title) {
        return repository.findByTitleContainingIgnoreCase(title);
    }
    public byte[] getByIdAndFileIndex(Long id, int fileIndex) {
        NpartLock entity = getById(id);
        switch (fileIndex) {
            case 1:
    ...
    public NpartLock getById(Long id) {
        return repository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException());
    }


    public NpartLock save(NpartLockDTO dto) throws IOException {
        NpartLock entity = new NpartLock();
    ...
    public NpartLock update(Long id, NpartLockDTO dto) throws IOException {
        NpartLock entity = getById(id);
        entity.setTitle(dto.getTitle());
        entity.setMaterial(dto.getMaterial());
        entity.setSurface(dto.getSurface());
