...
import com.keyworld.projectboard.dto.DimpleLockEngDTO;
import com.keyworld.projectboard.repository.DimpleLockEngRepository;
...
@Service
public class DimpleLockEngService {

    @Autowired
    private DimpleLockEngRepository repository;

    public List<DimpleLockEng> getAll() {
        return repository.findAll();
    }

    public List<DimpleLockEng> findDimpleLockEngsByTitle(String title) {
        return repository.findByTitleContainingIgnoreCase(title);
    }
    public byte[] getByIdAndFileIndex(Long id, int fileIndex) {
        DimpleLockEng entity = getById(id);
        switch (fileIndex) {
            case 1:
    ...
    public DimpleLockEng getById(Long id) {
        return repository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException());
    }


    public DimpleLockEng save(DimpleLockEngDTO dto) throws IOException {
        DimpleLockEng entity = new DimpleLockEng();
    ...
    public DimpleLockEng update(Long id, DimpleLockEngDTO dto) throws IOException {
        DimpleLockEng entity = getById(id);
        entity.setTitle(dto.getTitle());
        entity.setMaterial(dto.getMaterial());
        entity.setSurface(dto.getSurface());
