...
@Service
public class CamLockEngService {

    @Autowired
    private CamLockEngRepository repository;

    public List<CamLockEng> getAll() {
        return repository.findAll();
    }

    public List<CamLockEng> findCamLockEngsByTitle(String title) {
        return repository.findByTitleContainingIgnoreCase(title);
    }
    public byte[] getByIdAndFileIndex(Long id, int fileIndex) {
        CamLockEng entity = getById(id);
        switch (fileIndex) {
            case 1:
    ...
    public CamLockEng getById(Long id) {
        return repository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException());
    }


    public CamLockEng save(CamLockEngDTO dto) throws IOException {
        CamLockEng entity = new CamLockEng();
    ...
    public CamLockEng update(Long id, CamLockEngDTO dto) throws IOException {
        CamLockEng entity = getById(id);
        entity.setTitle(dto.getTitle());
        entity.setMaterial(dto.getMaterial());
        entity.setSurface(dto.getSurface());
