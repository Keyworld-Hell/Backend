...
public class HandleService {
    ...
    public List<Handle> findHandlesByTitle(String title) {
        return repository.findByTitleContainingIgnoreCase(title);
    }
    public byte[] getByIdAndFileIndex(Long id, int fileIndex) {
        Handle entity = getById(id);
        switch (fileIndex) {
            case 1:
    ...
    public Handle save(HandleDTO dto) throws IOException {
    ...
    public Handle update(Long id, HandleDTO dto) throws IOException {
        Handle entity = getById(id);
        entity.setTitle(dto.getTitle());
        entity.setMaterial(dto.getMaterial());
        entity.setSurface(dto.getSurface());
