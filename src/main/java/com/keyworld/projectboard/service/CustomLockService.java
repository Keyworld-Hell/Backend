...
public class CustomLockService {
    ...
    @Autowired
    private CustomLockRepository repository;

    public List<CustomLock> getAll() {
        return repository.findAll();
    }

    public List<CustomLock> findCustomLocksByTitle(String title) {
        return repository.findByTitleContainingIgnoreCase(title);
    }
    public byte[] getByIdAndFileIndex(Long id, int fileIndex) {
        CustomLock entity = getById(id);
        switch (fileIndex) {
            case 1:
    ...
    public CustomLock getById(Long id) {
        return repository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException());
    }


    public CustomLock save(CustomLockDTO dto) throws IOException {
        CustomLock entity = new CustomLock();
    ...
    public CustomLock update(Long id, CustomLockDTO dto) throws IOException {
        CustomLock entity = getById(id);
        entity.setTitle(dto.getTitle());
        entity.setMaterial(dto.getMaterial());
        entity.setSurface(dto.getSurface());
