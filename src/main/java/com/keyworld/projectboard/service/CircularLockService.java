...
@Service
public class CircularLockService {

    @Autowired
    private CircularLockRepository repository;

    public List<CircularLock> getAll() {
        return repository.findAll();
    }

    public List<CircularLock> findCircularLocksByTitle(String title) {
        return repository.findByTitleContainingIgnoreCase(title);
    }
    public byte[] getByIdAndFileIndex(Long id, int fileIndex) {
        CircularLock entity = getById(id);
        switch (fileIndex) {
            case 1:
    ...
    public CircularLock getById(Long id) {
        return repository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException());
    }


    public CircularLock save(CircularLockDTO dto) throws IOException {
        CircularLock entity = new CircularLock();
    ...
    public CircularLock update(Long id, CircularLockDTO dto) throws IOException {
        CircularLock entity = getById(id);
        entity.setTitle(dto.getTitle());
        entity.setMaterial(dto.getMaterial());
        entity.setSurface(dto.getSurface());
